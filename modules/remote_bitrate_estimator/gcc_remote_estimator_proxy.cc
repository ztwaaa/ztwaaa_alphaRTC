...
const int GccRemoteEstimatorProxy::kMaxNumberOfPackets = (1 << 15);
...
GccRemoteEstimatorProxy::GccRemoteEstimatorProxy(
    Clock* clock,
...
void GccRemoteEstimatorProxy::IncomingPacket(int64_t arrival_time_ms,
                                          size_t payload_size,
...
bool GccRemoteEstimatorProxy::LatestEstimate(std::vector<unsigned int>* ssrcs,
                                          unsigned int* bitrate_bps) const {
  return false;
}

int64_t GccRemoteEstimatorProxy::TimeUntilNextProcess() {
  rtc::CritScope cs(&lock_);
  if (!send_periodic_feedback_) {
...
void GccRemoteEstimatorProxy::Process() {
  rtc::CritScope cs(&lock_);
  if (!send_periodic_feedback_) {
    return;
  }
...
void GccRemoteEstimatorProxy::OnBitrateChanged(int bitrate_bps) {
...
void GccRemoteEstimatorProxy::SetSendPeriodicFeedback(
    bool send_periodic_feedback) {
  rtc::CritScope cs(&lock_);
  send_periodic_feedback_ = send_periodic_feedback;
}

void GccRemoteEstimatorProxy::SendPeriodicFeedbacks() {
...
void GccRemoteEstimatorProxy::SendFeedbackOnRequest(
...
int64_t GccRemoteEstimatorProxy::BuildFeedbackPacket(
